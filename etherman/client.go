...
var (
	...
	depositTicketAddedSignatureHash  = crypto.Keccak256Hash([]byte("DepositTicketAdded(bytes32,address,uint256,bytes32)"))
	withdrawTicketAddedSignatureHash = crypto.Keccak256Hash([]byte("WithdrawTicketAdded(address,uint256,bytes32)"))
...
func NewClient(cfg Network) (*ethereumClient, error) {
	...
	if err != nil {
		fmt.Printf("chainID: %d, error connecting to %s: %+v\n", cfg.ChainId, cfg.Rpc, err)
		return nil, err
	}

	entryPoint, err := EntryPoint.NewEntryPoint(cfg.EntryPoint, ethClient)
	if err != nil {
		return nil, fmt.Errorf("chainID: %d, failed to init entryPoint: %w", cfg.ChainId, err)
	}

	accountFactory, err := SimpleAccountFactory.NewSimpleAccountFactory(cfg.AccountFactory, ethClient)
	if err != nil {
		return nil, fmt.Errorf("chainID: %d, failed to init simple account factory: %w", cfg.ChainId, err)
	}
...
func (c *ethereumClient) readEvents(ctx context.Context, query ethereum.FilterQuery) ([]Block, error) {
	...
	for _, vLog := range logs {
		...
		if err != nil {
			fmt.Printf("error processing event. Retrying... Error: %s. vLog: %+v\n", err.Error(), vLog)
			return nil, err
		}
	}
	return blocks, nil
...
func (c *ethereumClient) accountCreateEvent(vLog types.Log, blocks *[]Block) error {
	fmt.Println("AccountCreate event detected")
	...
	if len(*blocks) == 0 || ((*blocks)[len(*blocks)-1].BlockHash != vLog.BlockHash || (*blocks)[len(*blocks)-1].BlockNumber != vLog.BlockNumber) {
		...
		*blocks = append(*blocks, block)
		...
		fmt.Println("Error processing accountCreate event. BlockHash:", vLog.BlockHash, ". BlockNumber: ", vLog.BlockNumber)
...
func (c *ethereumClient) depositEvent(vLog types.Log, blocks *[]Block) error {
	fmt.Println("Deposit event detected")
	...
	if len(*blocks) == 0 || ((*blocks)[len(*blocks)-1].BlockHash != vLog.BlockHash || (*blocks)[len(*blocks)-1].BlockNumber != vLog.BlockNumber) {
		...
		*blocks = append(*blocks, block)
		...
		fmt.Println("Error processing deposit event. BlockHash:", vLog.BlockHash, ". BlockNumber: ", vLog.BlockNumber)
...
func (c *ethereumClient) withdrawEvent(vLog types.Log, blocks *[]Block) error {
	fmt.Println("Withdraw event detected")
	...
	if len(*blocks) == 0 || ((*blocks)[len(*blocks)-1].BlockHash != vLog.BlockHash || (*blocks)[len(*blocks)-1].BlockNumber != vLog.BlockNumber) {
		...
		*blocks = append(*blocks, block)
		...
		fmt.Println("Error processing Withdraw event. BlockHash:", vLog.BlockHash, ". BlockNumber: ", vLog.BlockNumber)
