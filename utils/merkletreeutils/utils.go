...
import (
	...
	"github.com/OAB/lib/common/length"
...
func KeyEthAddrBalance(ethAddr string) NodeKey {
	return Key(ethAddr, KeyBalance)
}

func KeyEthAddrNonce(ethAddr string) NodeKey {
	return Key(ethAddr, KeyNonce)
}

func KeyEthAddrNonceChainId(ethAddr string, nonceChainId string) NodeKey {
	ethAddrBigInt := ConvertHexToBigInt(ethAddr)
	ethAddrBigIngArray := ScalarToArrayBig(ethAddrBigInt)
	chainId, _ := StrValToBigInt(nonceChainId)
	chainIdArray, err := NodeValue8FromBigIntArray(ScalarToArrayBig(chainId))
	if err != nil {
		return NodeKey{}
	}

	hk0 := Hash(chainIdArray.ToUintArray(), [4]uint64{0, 0, 0, 0})

	key1 := NodeValue8{ethAddrBigIngArray[0], ethAddrBigIngArray[1], ethAddrBigIngArray[2], ethAddrBigIngArray[3], ethAddrBigIngArray[4], ethAddrBigIngArray[5], big.NewInt(int64(KeyNonce)), big.NewInt(0)}
...
func Key(ethAddr string, c int) NodeKey {
	...
	key1Capacity, err := StringToH4(HashPoseidonAllZeroes)
	if err != nil {
		return NodeKey{}
	}
...
func KeyContractStorage(ethAddr []*big.Int, storagePosition string) NodeKey {
	...
	key1 := NodeValue8{ethAddr[0], ethAddr[1], ethAddr[2], ethAddr[3], ethAddr[4], ethAddr[5], big.NewInt(int64(ScStorage)), big.NewInt(0)}
...
func HashContractBytecodeBigInt(bc string) *big.Int {
	...
	numHashes := int(math.Ceil(numBytes / (BytecodeElementsHash * BytecodeBytesElement)))
...
func EncodeKeySource(t int, accountAddr common.Address, chainId common.Hash) []byte {
	var keySource []byte
	keySource = append(keySource, byte(t))
	keySource = append(keySource, accountAddr.Bytes()...)
	if t == KeyNonce {
		keySource = append(keySource, chainId.Bytes()...)
	}
	return keySource
}
