...
func TypedDataAndHash(typedData TypedData) ([]byte, string, error) {
	...
	rawDataHash, err := poseidon.HashMessage([]byte(rawData))
	if err != nil {
		return nil, "", err
	}

	return poseidon.H4ToBytes(rawDataHash), rawData, nil
}

// HashStruct generates a poseidon hash of the encoding of the provided data
func (typedData *TypedData) HashStruct(primaryType string, data TypedDataMessage) (hexutil.Bytes, error) {
	encodedData, err := typedData.EncodeData(primaryType, data, 1)
	if err != nil {
		return nil, err
	}
	encodedDataHash, err := poseidon.HashMessage(encodedData)
	if err != nil {
		return nil, err
	}

	return poseidon.H4ToBytes(encodedDataHash), nil
}
...
// TypeHash creates the poseidon hash of the data
func (typedData *TypedData) TypeHash(primaryType string) hexutil.Bytes {
	primaryTypeHash, _ := poseidon.HashMessage(typedData.EncodeType(primaryType))

	return poseidon.H4ToBytes(primaryTypeHash)
}
